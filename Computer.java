...
public class Computer implements Runnable {
    ...
    public Computer(String name, String ipAddress, int port, ConfigManager configManager) throws UnknownHostException, SocketException {
        this.name = name;
    ...
    private void handleReceivedMessage(String message, InetAddress sourceAddress, int sourcePort) {
        ...
        if (message.contains("original")) {
            try {
                sendMessage(sourceAddress.getHostAddress(), sourcePort, "Received your message: " + message, "reply");
            } catch (IOException e) {
    ...
    private void sendMessage(String destIp, int destPort, String message, String type) throws IOException {
        ...
        byte[] data = (name + " (" + type + ") says to " + destMac + ": " + message).getBytes();
        DatagramPacket packet = new DatagramPacket(data, data.length, destAddress, destPort);
        socket.send(packet);
        System.out.println(name + " sent (" + type + ") message to " + destIp + ":" + destPort + ": " + message);
    }
